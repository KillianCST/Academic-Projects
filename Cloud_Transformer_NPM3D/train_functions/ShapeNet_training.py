...
def train_shapenet(model, device, train_loader, val_loader,
    ...
    # Initialize the GradScaler for mixed precision training if using CUDA
    scaler = torch.cuda.amp.GradScaler() if device.type == "cuda" else None

    # Initial evaluation (using autocast for speed)
    model.eval()
    ...
    for epoch in range(epochs):
        ...
        model.train()
        train_loss = 0.0
        for batch in tqdm(_slice_loader(train_loader, subsample),
                          desc=f"Train — LR: {optimizer.param_groups[0]['lr']:.2e}",
                          total=num_train, leave=False):
            optimizer.zero_grad()
            ...
            with torch.cuda.amp.autocast(enabled=(scaler is not None)):
                ...
                # Combine losses (chamfer loss is scaled)
                loss = emd_loss * (1-chamfer_loss) + chamfer_weight * chamfer_loss
        ...
        sum_fwd = sum_bwd = sum_f1 = 0.0
        with torch.no_grad():
            for batch in tqdm(_slice_loader(val_loader, subsample),
                              desc=f"Val   — LR: {optimizer.param_groups[0]['lr']:.2e}",
                              total=num_val, leave=False):
                with torch.cuda.amp.autocast(enabled=(scaler is not None)):
                    pred   = model(batch['partial_noise'].to(device), batch['partial_enc'].to(device)).permute(0, 2, 1)
                    pred = pred.float()
        ...
        if (epoch + 1) % 5 == 0:
            backup_path = os.path.join(backup_dir, f"completion_shapenet_backup_epoch_{epoch+1}.pth")
            torch.save(model.state_dict(), backup_path)
            print(f"→ Backup saved: epoch {epoch+1} to {backup_path}")
